...
(function () {
  try {
    ...
    l.rel = "stylesheet"; l.href = "/css/v2.css?v=3.25.9";
    document.head.appendChild(l);
    // 2.0 排版純化：標題/導覽的開頭 emoji 在皮膚模式下移除（不動原始 HTML）
    document.addEventListener("DOMContentLoaded", () => {
      const EMO = /^[\s\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE0F}\u{200D}\u{20E3}]+/u;
      document.querySelectorAll(".page-head h1, .panel-head h2, .main-nav a, .panel h4").forEach(el => {
        for (const n of el.childNodes) {
          if (n.nodeType === 3 && n.textContent.trim()) { n.textContent = n.textContent.replace(EMO, ""); break; }
          if (n.nodeType === 1 && n.textContent.trim()) break;
        }
      });
    });
...
const STOCK_DATA = {
  ...
  news: [],   // 內建示意新聞已清空（2026-06-13）：一律讀 news_live.json 真資料，載入失敗則空白、不顯示假新聞
  hotTopics: [
...
function renderDate() {
  ...
  // ⚡ X 快訊雷達入口（桌機頂列，全站注入；手機 -m 頁沒有 .top-bar 自動跳過）
  try {
    const tl = document.querySelector(".top-bar .top-left");
    if (tl && !document.getElementById("xRadarTopLink")) {
      const a = document.createElement("a");
      a.id = "xRadarTopLink";
      a.href = "/pages/vip#x-radar";
      a.style.cssText = "color:#d6b36f;font-weight:800;margin-left:14px;text-decoration:none;display:inline-flex;align-items:center;gap:4px;";
      a.innerHTML = '<svg width="13" height="13" viewBox="0 0 256 256" fill="currentColor" style="flex:none;"><path d="M160,128a32,32,0,1,1-32-32A32,32,0,0,1,160,128Z" opacity=".35"/><path d="M128,88a40,40,0,1,0,40,40A40,40,0,0,0,128,88Zm0,64a24,24,0,1,1,24-24A24,24,0,0,1,128,152Zm73.71,7.14a80,80,0,0,1-14.08,22.2,8,8,0,0,1-11.92-10.67,63.95,63.95,0,0,0,0-85.33,8,8,0,1,1,11.92-10.67,80.08,80.08,0,0,1,14.08,84.47ZM69,103.09a64,64,0,0,0,11.26,67.58,8,8,0,0,1-11.92,10.67,79.93,79.93,0,0,1,0-106.67A8,8,0,1,1,80.29,85.34,63.77,63.77,0,0,0,69,103.09ZM248,128a119.58,119.58,0,0,1-34.29,84,8,8,0,1,1-11.42-11.2,103.9,103.9,0,0,0,0-145.56A8,8,0,1,1,213.71,44,119.58,119.58,0,0,1,248,128ZM53.71,200.78A8,8,0,1,1,42.29,212a119.87,119.87,0,0,1,0-168,8,8,0,1,1,11.42,11.2,103.9,103.9,0,0,0,0,145.56Z"/></svg>X 快訊雷達<span style="background:#d6b36f;color:#0b3328;font-size:10px;font-weight:900;padding:1px 6px;border-radius:8px;">NEW</span>';
      tl.appendChild(a);
    }
  } catch (e) {}
}
...
function renderTicker() {
  ...
  const items = topStocks.map(s => {
    ...
    return `<span class="tk-item" data-code="${s.code}">
      <a href="${_seoHref(s.code)}" style="color:inherit;text-decoration:none;">
...
function renderStockTable() {
  ...
  tbody.innerHTML = list.map(s => {
    ...
    return `<tr data-code="${s.code}">
      <td class="code"><a href="${_seoHref(s.code)}">${s.code}</a></td>
      <td class="name"><a href="${_seoHref(s.code)}" style="color:inherit;">${s.name}</a></td>
...
function bindMobileRowTap() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (code) location.href = _seoHref(code);
  });
...
function renderFairValueLow() {
  ...
  if (grid) {
    ...
    grid.innerHTML = top5.map(fv => {
      ...
      // 開關 OFF（現在）：全 open
      return `<a class="fv-low-card" href="${_seoHref(fv.code)}">
  ...
  if (sideUl) {
    sideUl.innerHTML = top5.map(fv => {
      ...
      const posLabel = fv.position <= 0 ? "低於區間下緣" : "合理區間下緣";
      const price = s ? fmtPrice(s.price) : "—";
      return `<li style="cursor:pointer;" onclick="location.href='${_seoHref(fv.code)}'">
...
function bindSearch() {
  ...
  function doSearch() {
    ...
    if (exact) { location.href = _seoHref(exact.code); return; }

    // 2. 完全符合公司名稱 → 跳個股
    const exactName = STOCK_DATA.stocks.find(s => s.name === q);
    if (exactName) { location.href = _seoHref(exactName.code); return; }

    // 3. 公司名稱開頭含查詢字串（例如「環球晶」→ 環球晶圓）→ 唯一結果直接跳
    const startsWith = STOCK_DATA.stocks.filter(s => s.name.startsWith(q));
    if (startsWith.length === 1) { location.href = _seoHref(startsWith[0].code); return; }
...
function setupThemeToggle() {
  ...
  // 注入到頁尾版權列（從上方列移到最底；喜氣紅是節慶換色的次要功能）
  document.querySelectorAll(".copyright .container").forEach(host => {
...
function setupLineFab() {
  if (document.querySelector(".line-fab")) return;
  const a = document.createElement("a");
  a.className = "line-fab";
  a.href = "https://line.me/R/ti/p/@130tqckv";
  a.target = "_blank";
  a.rel = "noopener";
  a.setAttribute("aria-label", "LINE 客服");
  a.title = "加 LINE 客服";
  a.innerHTML = `<span class="line-fab-icon" aria-hidden="true"><svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M19.365 9.863c.349 0 .631.285.631.631 0 .345-.282.63-.631.63H17.61v1.125h1.755c.349 0 .631.283.631.63 0 .344-.282.629-.631.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.63-.63h2.386c.346 0 .628.285.628.63 0 .349-.282.63-.628.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.63-.63.346 0 .628.285.628.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.348 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.282.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314"/></svg></span><span class="line-fab-label">LINE 客服</span>`;
...
function injectStockJsonLd() {
  ...
  const url = SITE_ORIGIN + _seoHref(code);   // og:url 指向 canonical SEO 頁（與 canonical 一致）
  const pct = pctChange(s.price, s.change);
  ...
  //   順序很重要：SERP 只顯示前 ~80 個全形字，所以「合理區間數字」與「不喊目標價
  //   的立場」必須排在最前面，其餘數據點（本益比、董事長等）當補充放後面，被截掉
  //   也不影響。descParts 是用「、」串接的短語清單，不能放完整句子。
  const _lead = `${s.name}（${s.code}）${_fvTxt}。想找 ${s.name} 目標價？`
    + `領富 AI 不喊分析師目標價，改用公開財報算出的合理區間，幫你判斷現在該不該買。`;
  let descParts = [`${marketLabel}・${s.category}類股`];
  ...
  descParts.push("每日更新，非投資建議");
  const seoDesc = (_lead + descParts.join("、") + "。").slice(0, 158);
...
function setupAiAlert() {
  ...
  if (itemsHost) {
    ...
    const items = topGainers.map((s, i) => {
      ...
      return `<a class="ai-alert-item" href="${_seoHref(s.code)}">
...
function lqBuildWatchlistRows() {
  ...
  container.innerHTML = `
    ...
    ${visible.map(it => `
      <a class="lq-w-row" data-code="${it.code}" data-market="${it.market}" href="${_seoHref(it.code)}">
...
const DATA_CACHE_NAME = "leadfu-data-v3259";
