...
//
//   2026-09-07 把整份 NIM 目錄（81 個）掃過一遍，實際能回繁中答案的只有 10 個
//   （其餘 55 個 404、5 個逾時，剩下是安全防護／翻譯／embedding，不是聊天模型）。
//   接著用真的 SYSTEM_PROMPT 各連發 5 次、間隔 8 秒：
//
//     模型                                 成功   中位     最慢
...
//                                                                 content 空，串流路徑偵測
//                                                                 不到，使用者會看到空白
//     minimaxai/minimax-m3                 1/5      —       —     ✘ 免費層限流太兇
//     nvidia/nemotron-3-super-120b-a12b     —       —       —     ✘ 短提示 1.1s 很快，但帶
//                                                                 7k 的 SYSTEM_PROMPT 直接 400
//     nvidia/ising-calibration-1.5-31b      —     4.9s      —     ✘ 沒資料時會自己編出價位
//     nvidia/nemotron-3.5-lightning-30b     —     4.9s      —     ✘ 把 thinking 寫進 content
//     poolside/laguna-xs-2.1                —    56.5s      —     ✘ 慢，而且答錯（把本益比
//                                                                 說成「每賣一台的利潤」）
...
- 完全依照使用者訊息裡指定的段落結構與標題輸出。`;
  ...
  // 呼叫端指定了就只打那一個、不替他換——他是在測那個模型。
  const callerModel = (typeof body.model === "string" && body.model.length < 200) ? body.model : "";
  const model = callerModel || env.NVIDIA_MODEL || DEFAULT_MODEL;
  ...
  // 依序試 MODEL_CHAIN，都不行才 fallback 到 Gemini。
  // ⚠ 換下一個模型的時機有兩種，兩種都要接：
  //   1. 429 —— NIM 免費層是**按模型**限流的，重打同一個永遠是 429。
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    try {
      ...
      aiResp = r;
      // 只有限流與伺服器錯誤值得換模型再試；400/401 這種再試幾次都一樣
...
/* L1：確定性禁用詞。
   ⚠ 設計原則：擋「行為」，不是擋「詞彙」。L1 是便宜的第一道濾網，
     只該攔下無爭議的違規；有歧義的交給 L3（Gemini 讀全文做語意判斷）。
     L1 攔得太寬的代價不是「比較安全」，而是稿子在 L3 之前就被殺掉，
     而且站長原本看不到理由（2026-08-31 三篇草稿卡住就是這樣來的）。

   兩處刻意放寬（2026-08-31）：
   1. 原本 /目標價/ 擋裸字 → 會把本站自己的編輯立場也擋掉。領富的立場正是
      「我們不喊目標價，而且要解釋為什麼目標價不可信」，個股頁標題就寫著
      「合理價與目標價」、FAQ 寫「不提供分析師式目標價」。要批評它就得說得出它。
      真正的風險是「本站印出一個具體價位」，所以改成帶數字才擋。
   2. 原本擋「值得關注／可留意／值得留意」→ 中文財經寫作裡這是中性的行文連接詞
      （實例：「值得關注是否代表真正的基本面反轉」＝分析語氣，不是推薦個股）。
      保留語氣明確的「值得買／建議關注／不妨留意」，其餘交給 L3。 */
const INSIGHT_L1_PATTERNS = [
  [/目標價[^。；\n]{0,14}?\d+(\.\d+)?\s*元/, "印出具體目標價"],
  [/(上看|喊到|挑戰至|直指)\s*\d+(\.\d+)?\s*元/, "喊出價位"],
...
async function insightWriteArticle(env, b) {
  ...
  const sys = [
    ...
    "任務：把一份『選題大綱』擴寫成一篇 1600-2400 字的繁體中文深度分析文。",
    "",
    ...
    "2. 【長度怎麼來】深度來自『把大綱每個要點講透』——背景脈絡、運作機制、",
    "   會影響誰、要看什麼指標才知道兌現、反方怎麼說、對散戶的實際意義。",
    "   絕不是靠加入新事實或湊字數；寧可把既有要點寫深，也不要編造。",
    "3. 不預測股價、不建議買賣、不點名要買哪一檔。可以說明產業趨勢與風險。",
    "4. 提到風險時要具體，不要只寫「投資有風險」。",
    "5. 用 Markdown 結構（有利閱讀與搜尋）：",
    "   - 開頭 2-3 段導言，說清楚「這件事為什麼現在重要」",
    "   - 中間 4-6 個 ## 小標，每段 250-400 字，一個小標講一件事",
    "   - 一個 ## 反方觀點／風險（務必獨立成段，不可省略）",
    "   - 結尾 ## 這對散戶的意義：可以觀察什麼、什麼情況代表題材沒兌現",
    "6. 最後一行固定加：> 本文由 AI 依公開資料整理、經自動查核與人工監督發佈，僅供研究參考，不構成投資建議。",
    "7. 全文繁體中文，語氣平實，不用驚嘆號。"
  ].join("\n");
  const usr = [
    `【標題方向】${b.title_hint || ""}`,
...
async function insightL3Gate(env, b, article) {
  const sys = [
    ...
    "",
    "【必須否決的情況】",
    "A. 叫讀者去買賣或關注某一檔個股（包含委婉的：逢低、進場、不妨留意某檔…）。",
    "   ※ 注意分辨：『值得關注這波漲價是否具延續性』是分析語氣，不算違規；",
    "     『XX 值得關注』把矛頭指向特定個股才算。判斷「有沒有叫人去做什麼」，不是看詞。",
    "B. 本文印出任何具體目標價價位（例如「目標價 120 元」「上看 150 元」）。",
    "   ※ 討論或批評『目標價』這個概念、或客觀陳述『外資上修目標價』而不寫出數字，不算違規——",
    "     本站的編輯立場本來就是不喊目標價並說明理由，要講這件事就得說得出這三個字。",
...
async function renderStockPage(url, env) {
  ...
  if (!s) {
    const nf = `<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>找不到代號 ${esc(code)} - 領富 AI</title><meta name="robots" content="noindex"><link rel="stylesheet" href="/css/style.css?v=3.23.9"></head><body><div style="max-width:600px;margin:80px auto;padding:24px;text-align:center;font-family:sans-serif;"><h1>找不到「${esc(code)}」這檔股票</h1><p style="color:#666;">可能代號有誤，或不在追蹤範圍。<br><a href="/pages/stocks" style="color:#1B4332;">← 回股價總覽</a>　<a href="/pages/check" style="color:#1B4332;">用買前檢查器查詢</a></p></div></body></html>`;
  ...
  // 興櫃另走一套標題／描述。GSC 90 天實測：興櫃股票的查詢有一大塊是在問
  // 「現在到底在興櫃、上櫃還是上市」——38 個查詢、844 曝光、排名 5–10 名，
  // 但 **0 點擊**。頁面 FAQ 其實答了，可是描述整段在講合理價與目標價，
  // 摘要看起來不像會回答他的問題，所以沒人點。
  //   實際查詢：「芯測 6786 興櫃 還是 上櫃 上市」「6786 芯測 興櫃 上櫃 上市」…
  // 這個模式只在興櫃成立（過渡狀態才會被問「還在嗎」），上市／上櫃不套用。
  const isEmerging = market === "興櫃";
  const title = isEmerging
    ? `${name} ${code} 興櫃還是上櫃？合理價與目標價 - 領富 AI`
    : `${name} ${code} 合理價與目標價｜買前風險檢查 - 領富 AI`;
  ...
  // 興櫃：先答「現在在哪個市場」，那是這批頁面實際被搜的問題；合理區間接在後面。
  // 其餘市場別維持原本以合理價／目標價開頭（那才是它們的查詢主力）。
  const desc = isEmerging
    ? `${name}（${code}）目前為「興櫃」股票，尚未轉上櫃或上市——轉板需經櫃買中心審查，進度以 TPEx 與公開資訊觀測站公告為準。${fvTxt}${score != null ? "、防錯雷達風險 " + score + "/100" : ""}。${cat ? cat + "類股，" : ""}每日更新、免費，非投資建議。`
    : `${name}（${code}）${fvTxt}${score != null ? "、防錯雷達風險 " + score + "/100" : ""}。想找 ${name} 目標價？領富 AI 不喊分析師目標價，改用公開財報算出的合理區間，幫你判斷現在該不該買。${cat ? cat + "類股，" : ""}每日更新、免費，非投資建議。`;
