...
def render(cat, slug, intro, lst, risk):
    ...
    for s in lst_sorted:
        ...
        if level and level != "低":
            ...
            rtext = ""
        rows.append(
            f'<tr><td><a href="../stock-detail?code={code}">{code}</a></td>'
            f'<td><a href="../stock-detail?code={code}">{name}</a></td>'
            f'<td class="c">{mkt}</td>'
...
<nav class="main-nav">
  <div class="container">
    <ul>
      ...
      <li><a href="../learn.html">📚 學習中心</a></li>
...
<main class="ind-wrap">
  ...
  <div class="ind-tools">
    <a href="../check.html">🔍 個股買前檢查</a>
    <a href="../risk-radar.html">⚠ 防錯雷達</a>
    <a href="../learn/warning-stocks.html">📖 注意股是什麼</a>
  </div>

  <h2 style="font-size:19px;color:#1B4332;margin:24px 0 10px;">{escape(cat)}成分股清單</h2>
  <div class="ind-src">📅 更新日期：{TODAY}　·　資料來源：TWSE 證交所、TPEx 櫃買中心、MOPS 公開資訊觀測站（風險等級為領富 AI 依公開資料整理）</div>
