...
判定
----
連問 5 次，至少要有 3 次答得出來（60%）才算通過。

為什麼不是「有一次過就好」：主力掛掉時 Gemini 備援還在，但免費額度是按
分鐘限流的，所以會變成「偶爾答得出來、多數回 502」。2026-09-07 早上實測
就是這樣——單發一次探測會矇混過關（第 3 次才矇到），連發多次才看得出來
已經壞了。使用者體感也是這個：十次有七次拿不到回應，那就是壞了。

為什麼不是「每次都要過」：上游是免費層，本來就會零星失敗。修好之後實測
穩定在 5/6 左右，用 100% 當門檻等於天天亮紅燈，看久了就會被無視——
會叫的警報要留給真的壞掉的時候。60% 這條線分得開兩種狀態：
壞掉時是 1/6（17%），正常時是 5/6（83%）。

沒過就推 LINE 給站長並 exit 1，讓 workflow 亮紅燈。
...
def main():
    ...
    ok = ATTEMPTS - len(fails)
    if ok >= MIN_OK:
        print(f"\n✅ AI 金絲雀通過：{ok}/{ATTEMPTS} 次答得出來（門檻 {MIN_OK}）")
        return 0

    summary = (f"{ATTEMPTS} 次探測只成功 {ok} 次（門檻 {MIN_OK}）。"
               f"最後一次失敗：{fails[-1]}")
    print(f"\n❌ AI 金絲雀失敗：{summary}")
